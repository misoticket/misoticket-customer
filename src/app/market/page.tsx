...
export default function Page() {
    ...
    useEffect(() => {
        if (selectedCategory && marketPrices !== null) {
            let newList = [];

            for (const cat of marketPrices.categoryList.filter(
                (prodOrderList) =>
    ...
    async function fetchData() {
        ...
        for (const cat of marketPrices.categoryList.filter(
            (prodOrderList) => prodOrderList.categoryId === catList[0].id
        )) {
            for (const proId of cat.productIdList) {
    ...
    function getCustomerSaleRatio(product: Product): number {
        return (
            Math.round(
                (100 -
                    (product.customerSellingPrice / product.originalPrice) *
                        100) *
                    10
            ) / 10
        );
    ...
    function getSellerSaleRatio(product: Product): number {
        return (
            Math.round(
                (100 - (product.price / product.originalPrice) * 100) * 10
            ) / 10
        );
    }
                ...
                <div>
                    ...
                    <div className="flex flex-col mb-24">
                        ...
                        <div className="flex flex-col">
                                ...
                                <div className="flex overflow-scroll scrollbar-hide">
                                    {categoryList.map((category) => (
                                        <p
                                            ...
                                            className={`font-medium whitespace-nowrap text-sm py-4 px-3 cursor-pointer hover:bg-gray-100 ${
                                                selectedCategory &&
                                                selectedCategory.id ===
                                                    category.id
                                                    ? "text-theme-sub"
                            ...
                            <p className="mx-4 text-center mt-4 font-medium text-theme text-xs">
                                ※ 구겨지거나 선물용으로 부적합한 상품권은
                                매입가가 변경될수 있습니다. (구권포함)
                            </p>
                            <div className="flex mt-10 mx-4 border-t border-b border-solid border-gray-200">
                                ...
                                <div className="h-14 w-48 bg-gray-50 flex flex-col justify-center border-r border-solid border-gray-200">
                                    ...
                                    <p className="font-medium text-xxs text-center text-gray-400 mt-0.5">
                                        (할인율 %)
                                    </p>
                                </div>
                                <div className="h-14 w-48 bg-gray-50 flex flex-col justify-center border-r border-solid border-gray-200">
                                    ...
                                    >
                                        ...
                                        <div className="h-12 w-48 border-r border-solid border-gray-200 flex flex-col justify-center items-center group-hover:bg-gray-100 bg-yellow-100">
                                            <p className="font-bold text-sm text-center">
                                                {product.customerSellingPrice.toLocaleString()}
                                                <span className="font-medium">
                                                    원
                                                </span>
                                            </p>
                                            <p className="font-medium text-xs text-center text-theme-sub mr-2">
                                                {getCustomerSaleRatio(product)}%
                                            </p>
                                        </div>
                                        <div className="h-12 w-48 border-r border-solid border-gray-200 flex flex-col justify-center items-center group-hover:bg-gray-100">
                                            ...
                                            <p className="font-medium text-xs text-center text-theme mr-2">
                                                {getSellerSaleRatio(product)}%
                                            </p>
                                        </div>
                                    </div>
                                ))}
                ...
                <div>
                    ...
                    <div className="flex flex-col items-center py-4">
                        ...
                        <div className="flex flex-col">
                                ...
                                <div className="flex">
                                    {categoryList.map((category) => (
                                        <p
                                            ...
                                            className={`font-medium text-sm py-4 px-3 cursor-pointer hover:bg-gray-100 ${
                                                selectedCategory &&
                                                selectedCategory.id ===
                                                    category.id
                                                    ? "text-theme-sub"
                            ...
                            <p className="text-center mt-4 font-medium text-theme text-xs">
                                ※ 구겨지거나 선물용으로 부적합한 상품권은
                                매입가가 변경될수 있습니다. (구권포함)
                            </p>
                            <div className="flex w-full mt-10 border-t border-b border-solid border-gray-200">
                                ...
                                <div className="h-14 w-56 bg-gray-50 flex flex-col justify-center border-r border-solid border-gray-200">
                                    ...
                                    <p className="font-medium text-xs text-center text-gray-400 mt-0.5">
                                        (할인율 %)
                                    </p>
                                </div>
                                <div className="h-14 w-56 bg-gray-50 flex flex-col justify-center border-r border-solid border-gray-200">
                            ...
                            <div>
                                {showProductList.map((product) => (
                                    ...
                                    >
                                        <div className="h-12 w-full flex flex-col justify-center border-l border-r border-solid border-gray-200 group-hover:bg-gray-100">
                                            <p className="font-medium text-sm mx-5">
                                                {product.name}
                                            </p>
                                        </div>
